...
data.real <- read.csv("numero.matriculas_IFDM_e_FPM_agregados.csv", head = T, stringsAsFactors=F,dec = ".")
...
#Fun��o que recebe o nome de uma cidade e retorna as n cidades mais proximas(10 � o padr�o)
calcDistanciaCidadesSemelhantes <- function(nome.cidade, quant.cidades = 10) {
  ...
  if(nome.cidade %in% data.real$NOME_MUNICIPIO == F) {
    ...
    outras.cidades <- subset(data, data$NOME_MUNICIPIO != nome.cidade & data$FPM == cidade$FPM)  
...
calcMediaDistanciaCidadesSemelhantes = function(nome.cidade, quant.cidades = 10) {
  linha = calcDistanciaCidadesSemelhantes(nome.cidade, quant.cidades)[,2]
  linha = na.omit(linha)
  if(length(linha) == 0) {
    return (-1)
  }
...
write.csv(semelhantes_nomes, "cidades_semelhantes_nomes_com_FPM.csv", row.names = F)

####lista com distancia das cidades semelhantes####
semelhantes_distancias = calcTodasDistanciasCidadesSemelhantes(data, nomes = F)
write.csv(semelhantes_distancias, "cidades_semelhantes_distancias_com_FPM.csv", row.names = F)

####Ordenar de acordo com a maior distancia media####
cidades_distancias = read.csv("cidades_semelhantes_distancias_com_FPM.csv", head = T, dec = ".")
cidades_distancias = cidades_distancias[with(cidades_distancias,order(cidades_distancias$V12)), ]
write.csv(cidades_distancias,"cidades_semelhantes_distancias_com_FPM.csv",row.names = F)
